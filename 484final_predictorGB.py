...
def multinomial_bayes(x_data,y_label,p,g_truth,titles):
    MB = MultinomialNB()
    MB.fit(x_data,y_label)
    pred  = MB.predict(p)
    score = f1_score(g_truth,pred, average='weighted')  
    with codecs.open('results_MB.csv','w','utf-8') as outfile:
...
if __name__ == "__main__":
    ...
    print(testing.shape)
    multinomial_bayes(training,train_y,testing,g_truth,titles)
