...
def Pre(par):
    ...
    paragraph = re.sub('[^+A-Za-z0-9]+',' ',paragraph) # substitutes non alphabetic or numerical characters with a space
    ...
    if '' in paragraph: # removes any empty character within a passed paragraph that may be left over
        while '' in paragraph:
            paragraph.remove('')
    ...
    new_paragraph = [word[0] for word in paragraph if((word[0] not in stpwrd) and (word[1] in pos_keep))] # the first condition is uneccesary if the second works properly
